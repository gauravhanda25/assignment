...
commonQuery.updateOneDocumentWithOutInserting = (model, updateCond, updateData) => {
     ...
     * 
     */
commonQuery.updateOneDocumentWithoutSet = (model, updateCond, updateData) => {
    return new Promise((resolve, reject) => {
        model.findOneAndUpdate(updateCond, updateData).exec((err, updatedData) => {
            if (err) {
                console.log(err);
                return reject(0);
            } else {
